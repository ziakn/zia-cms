<template>
	<v-main>
		...
		<v-btn bottom color="purple darken-3" dark fab fixed right :to="handleGoToMenu('/staff/addproperty/')">
		...
		async save() {
			...
			if (this.editedIndex > -1) {
				try {
					let { data } = await axios({
						method: "put",
						url: "/app/category/" + this.dataList[this.editedIndex].id,
